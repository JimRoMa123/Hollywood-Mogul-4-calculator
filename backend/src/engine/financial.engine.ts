...
export const FAMILY_GENRES = ['Family', 'Animation', 'Comedy', 'Musical'];
...
export class FinancialEngine {
  calculate(dto: CalculateMovieDto): {
    ...
    const totalTechnicalPct =
      dto.vfxBudgetPct + dto.creatureFxPct + dto.stuntsPct;
    ...
    const franchiseBonus = dto.franchiseMode
      ? (dto.franchiseMomentum || 0) * 0.15
      : 0;
    ...
    const genreMultiplier = isTechnicalGenre
      ? 3.5
      : AWARDS_FRIENDLY_GENRES.includes(dto.genre)
        ? 2.0
        : 2.5;
